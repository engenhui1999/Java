...
public class Main {
  ...
  public static void main(String[] args) {
    ArrayList<CompletableFuture<String>> array = new ArrayList<CompletableFuture<String>>();
    ...
    try {
      ...
      while (sc.hasNext()) {
        ...
        array.add(BusSg.findBusServicesBetween(srcId, searchString)
                     .thenCompose(input -> input.description()));
      }
      sc.close();
      CompletableFuture<?>[] newArr = new CompletableFuture<?>[array.size()];
      array.toArray(newArr);
      CompletableFuture.allOf(newArr).join();
      for (CompletableFuture<?> elem : newArr) {
        elem.thenAccept(x -> System.out.println(x));
      }
